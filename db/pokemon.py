...
class PokemonService(BaseService):
    async def create_pokemon(self, number, name, image_url, message_id, catch_count):
        document: PokemonDoc = {
            "number": number,
            "name": name,
            "image_url": image_url,
    ...
    async def user_has_pokemon(self, user_id, number):
        pokemon = self.col.find_one({"caught_by": user_id, "number": number})
        return pokemon is not None

    async def get_pokemon_by_message_id(self, message_id: str) -> Optional[PokemonDoc]:
        pokemon: Optional[PokemonDoc] = self.col.find_one({"message_id": message_id})
        return pokemon

    async def add_catch_attempt(self, message_id, user, pokemon) -> bool:
    ...
    def catch_pokemon(self, message_id, pokemon, user) -> bool:
        result = self.col.update_one(
            {"message_id": message_id, "_v": pokemon["_v"]},
            {
                "$set": {
    ...
    async def get_my_caught_pokemon(self, user) -> List[PokemonDoc]:
        pokemon_list: List[PokemonDoc] = list(self.col.find({"caught_by": user.id})) or []
        return pokemon_list

    async def get_pokemon(self, user, number):
        pokemon = self.col.find_one({"number": number, "caught_by": user.id})
        return pokemon if pokemon and pokemon["caught"] else False

    async def trade_pokemon(self, user1, user2, pokemon1, pokemon2):
